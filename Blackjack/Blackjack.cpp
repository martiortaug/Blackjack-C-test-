...
const int suit = 4;
const int card = 13;
const char* signs[] = { "\x03", "\x04", "\x05", "\x06" }; // Extended ASCII codes for card suits
int deck[suit][card];
int cardCount = 52;
...
void showDrawnCard(string player, int card, int cardValue, int suit) {
	...
	if (player == dlName && hideFirstCard) {
		...
		if (card == 0 || card > 10) {
			...
			switch (suit) {
			case 0:
				cout << "♥ " << "(" << cardJQK << " of hearts).\n";
				break;
			case 1:
				cout << "♦ " << "(" << cardJQK << " of diamonds).\n";
				break;
			case 2:
				cout << "♣ " << "(" << cardJQK << " of clovers).\n";
				break;
			case 3:
				cout << "♠ " << "(" << cardJQK << " of spades).\n";
				break;
			}
		}
		else {
...
int main() {
	// Configurar la consola para que soporte caracteres Unicode
